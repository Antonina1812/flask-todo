...
## 4. Я иногда отходу от ноута, он в спящем режиме, после того как захожу и перезагружаю графану все слетает и мне приходится логиниться заново, но при этом дашборды не сохраняются

**РЕШЕНИЕ**
Добавляем в docker-compose.yml в сервис графаны строчки:
volumes:
    - grafana-data:/var/lib/grafana    # добавить volume
environment:
    - GF_SECURITY_ADMIN_PASSWORD=admin  # зафиксировать пароль

## 5. Было неудобно писать код на маке, потом пушить, потом пулить на вм

**РЕШЕНИЕ**
Устанавливаем расширение для VS Code "Remote - SSH". Подключаемся по ssh к вм. Получаем терминал вм и полный доступ ко всем дикториям пользователя.